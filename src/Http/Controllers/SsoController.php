...
namespace BuiltByOtte\Sso\Http\Controllers;
...
class SsoController 
{
    ...
    public function webhook(Request $request)
    {
        ...
        return response(['success' => true, 'redirect' => route('sso.login', $this->generateToken($request->input('user_id')))], 200);
    }
    ...
    {
        $token = Str::random(config('sso.token.length', 48));
        Cache::add($token, $user_id, config('sso.token.lifetime', 60));
        return $token;
    }
